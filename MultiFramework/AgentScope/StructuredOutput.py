...
def get_vote_model(agents: list) -> type[BaseModel]:
    """获取投票阶段的输出格式"""
    agent_names = [_.name for _ in agents]
    name_choices = "、".join(agent_names)

    class VoteModel(BaseModel):
        """投票阶段的输出格式"""
        vote: str = Field(
            description=f"你要投票淘汰的玩家姓名，可选：{name_choices}",
        )
        reason: str = Field(
...
def get_seer_model(agents: list) -> type[BaseModel]:
    """获取预言家阶段的输出格式"""
    agent_names = [_.name for _ in agents]
    name_choices = "、".join(agent_names)
    ...
    class SeerModel(BaseModel):
        """预言家阶段的输出格式"""
        target: str = Field(
            description=f"要查验的玩家姓名，可选：{name_choices}",
        )
...
def get_hunter_model(agents: list) -> type[BaseModel]:
    """获取猎人阶段的输出格式"""
    agent_names = [_.name for _ in agents]
    name_choices = "、".join(agent_names)
    ...
    class HunterModel(BaseModel):
        ...
        target: Optional[str] = Field(
            description=f"要射杀的玩家姓名（可选：{name_choices}），如果不开枪则为None",
            default=None,
        )
