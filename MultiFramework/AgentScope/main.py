...
class ThreeKingdomsWerewolfGame():
    ...
    async def werewolf_phase(self, rount_num: int):
        """ 狼人阶段 - 展示消息驱动的协作模式 """
        ...
        async with MsgHub(
            ...
            announcement=await self.moderator.announce(
                f"狼人们，请讨论今晚的击杀目标。存货玩家：{format_player_list(self.alive_players)}"
            ),
        ) as werewolves_hub:
            # 讨论阶段：狼人通过消息交换策略
            ...
            if witch_action.metadata.get("use_antidote") and self.witch_has_antidote:
                ...
                if witch_action.metadata.get("use_poison") and self.witch_has_poison:
                    ...
                    if poisoned_player:
                        ...
                        await witch_agent.observe(await self.moderator.announce(f"你使用毒药杀死了{poisoned_player}"))
    ...
    async def hunter_phase(self, shot_by_hunter: str):
        ...
        if hunter_agent.name == shot_by_hunter:
            ...
            hunter_action = await hunter_agent(structured_model=get_hunter_model(self.alive_players))
    ...
    async def day_phase(self, round_num: int):
        ...
        async with MsgHub(
            ...
            vote_msgs = await fanout_pipeline(
                ...
                structured_model=get_vote_model(self.alive_players),
                enable_gather=False,
            )
